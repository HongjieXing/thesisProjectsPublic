...
# spaces = [50, 150, 250, 350, 450]
zcenter = []
for space in spaces:
    lampInfo = []
    for n in range (0, numLamps):
        x = -4 * lampSpace + (n % 9) * lampSpace
        y = 0.0
        z = (space+lampL)/2.0 - (n//9) * (space+lampL)
        angle_z2x, angle_z2y = 0.0, 0.0
        linfo = [lampL, x, y, z, angle_z2x, angle_z2y]
        lampInfo.append(linfo)
    zcenter.append((space+lampL)/2.0)
    # receiverYdis = space
...
for i, info in enumerate(infos):
    ...
    # plot LPF
    # plotFieldWithBox(matrixLPF, halfWidth, info = r"d$\mathit{z}$ = "+str(spaces[i]) + ' mm', zcenter = zcenter[i])
...
for i in range(0, out.shape[0]):
    subField, aveField, stdField = calcuSubField(out[i, 0, :, :], halfWidth, mode='vertical',zcenter=zcenter[i])
    stdValues.append(stdField)
    aveValues.append(aveField)
    # print(f"{spaces[i]:.2f}: mean = {aveField:.4f}, std={stdField:.4f}")
    # plot Rad:
    # plotFieldWithBox(out[i, 0, :, :], halfWidth, mode='Rad', info = r"d$\mathit{z}$ = "+str(spaces[i]) + ' mm', zcenter = zcenter[i])
...
ax.set_xlabel('Layer space' + ' / mm', fontsize=34)
...
plt.subplots_adjust(left=0.08,right=0.96,bottom=0.12,top=0.98)
